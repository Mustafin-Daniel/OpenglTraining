...
public class Sphere {
    ...
    private int numVert = 128; //Number of vertices in a full circle
    ...
    public Sphere(float x, float y, float z, float radius){
        ...
        for(int i=0;i<numVert;i++){
            ...
            float[] oneDot = {
                    x+(float)(radius*sin(PI*1/numCircles)*sin(2*PI*i/numVert)),
                    y+(float)(radius*sin(PI*1/numCircles)*cos(2*PI*i/numVert)),
                    z-(float)(radius*cos(PI*1/numCircles))
            };
            float[] twoDot = {
                    x+(float)(radius*sin(PI*1/numCircles)*sin(2*PI*(i+1)/numVert)),
                    y+(float)(radius*sin(PI*1/numCircles)*cos(2*PI*(i+1)/numVert)),
                    z-(float)(radius*cos(PI*1/numCircles))
            };
            tempC.add(bottom[0]); tempC.add(bottom[1]); tempC.add(bottom[2]);
            tempC.add(oneDot[0]); tempC.add(oneDot[1]); tempC.add(oneDot[2]);
            tempC.add(twoDot[0]); tempC.add(twoDot[1]); tempC.add(twoDot[2]);
            ...
            float[] normalVector = calculateNormal(a, b, oneDot, new float[]{x,y,z});

            normalX = normalVector[0]; normalY=normalVector[1]; normalZ=normalVector[2];
            tempNormals.add(normalX); tempNormals.add(normalY); tempNormals.add(normalZ);
            tempNormals.add(normalX); tempNormals.add(normalY); tempNormals.add(normalZ);
            tempNormals.add(normalX); tempNormals.add(normalY); tempNormals.add(normalZ);
        }
        for(int i=0;i<numVert;i++){
        ...
        for(int j=1;j<numCircles-2;j++){
            for(int i=0;i<numVert;i++){
                float[] oneDot = {
                        x+(float)(radius*sin(PI*j/numCircles)*sin(2*PI*i/numVert)),
                        y+(float)(radius*sin(PI*j/numCircles)*cos(2*PI*i/numVert)),
                        z-(float)(radius*cos(PI*j/numCircles))
                };
                float[] twoDot = {
                        x+(float)(radius*sin(PI*j/numCircles)*sin(2*PI*(i+1)/numVert)),
                        y+(float)(radius*sin(PI*j/numCircles)*cos(2*PI*(i+1)/numVert)),
                        z-(float)(radius*cos(PI*j/numCircles))
                };
                float[] threeDot = {
                        x+(float)(radius*sin(PI*(j+1)/numCircles)*sin(2*PI*i/numVert)),
                        y+(float)(radius*sin(PI*(j+1)/numCircles)*cos(2*PI*i/numVert)),
                        z-(float)(radius*cos(PI*(j+1)/numCircles))
                };
                float[] fourDot = {
                        x+(float)(radius*sin(PI*(j+1)/numCircles)*sin(2*PI*(i+1)/numVert)),
                        y+(float)(radius*sin(PI*(j+1)/numCircles)*cos(2*PI*(i+1)/numVert)),
                        z-(float)(radius*cos(PI*(j+1)/numCircles))
                };
                tempC.add(oneDot[0]); tempC.add(oneDot[1]); tempC.add(oneDot[2]);
                tempC.add(twoDot[0]); tempC.add(twoDot[1]); tempC.add(twoDot[2]);
                ...
                float normalX = normalVector[0]; float normalY=normalVector[1]; float normalZ=normalVector[2];
                tempNormals.add(normalX); tempNormals.add(normalY); tempNormals.add(normalZ); tempNormals.add(normalX); tempNormals.add(normalY); tempNormals.add(normalZ);
                tempNormals.add(normalX); tempNormals.add(normalY); tempNormals.add(normalZ); tempNormals.add(normalX); tempNormals.add(normalY); tempNormals.add(normalZ);
                tempNormals.add(normalX); tempNormals.add(normalY); tempNormals.add(normalZ); tempNormals.add(normalX); tempNormals.add(normalY); tempNormals.add(normalZ);
            }
        }
